...
// height
z = 10; //0.1
// bottom (negative: web at z/2.  Needs thicker wall)
bottom = 0; // 0.1
//round bottom
round_bottom = false;

// corner factor -- try 0 to sharpen the corners
c_scale = 1; //0.1
// corner add -- try 3 to add 3 to the radiuses of the corners
c_offset = 0; //0.1

// "Comment: Use c_scale=0 and c_offset=2 to override the 0,10,10,5mm radius corners to all be 2mm."
Comment = ".";
...
// x,y,radius
radiiPoints=[
  [0,  0,  0 * c_scale + c_offset   ],
  [x,  0,  10 * c_scale + c_offset],
  [x,  y,  10 * c_scale + c_offset],
  [0,  y,  5 * c_scale + c_offset ]
];

// use <Round-Anything-1.0.4/polyround.scad> // https://github.com/Irev-Dev/Round-Anything

// Version of Round-Anything included below for use in Thingiverse Customizer


module thinwall_rounded(){
    linear_extrude(z){
      shell2d(-thick)polygon(polyRound(radiiPoints,$fn));
    }
    if(bottom > 0) {
        if (! round_bottom){
        polyRoundExtrude(radiiPoints,0,0,bottom,fn=$fn);
        } else {
        
        // or do a rounded-off bottom with:
         translate([0,0,-(bottom-0.0001)]) polyRoundExtrude(radiiPoints,bottom,0,bottom,fn=$fn);
    }
}
    if (bottom < 0) // web in middle
      translate([0,0,(z- -bottom)/2])polyRoundExtrude(radiiPoints,-bottom,0,0,fn=$fn);
}
