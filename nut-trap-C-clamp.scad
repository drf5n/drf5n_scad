 /* 
 Nut-trap C-Clamp
 https://www.thingiverse.com/thing:5567280
 CC-BY-SA David Forrest 2022-10-15
 
 Round-Anything/polyround.scad library MIT 
...
/* [Main dimensions] */
nutname = "M3"; // [M1.6, M2, M2.5, M3, M4, M5, M6, M8, M10, M12, M14, M16, M20, M30 , M36, #0-80, #1-64, #2-56, #3-48, #4-40, #6-32, #8-32, #10-24, #12-24, 1/4-20, 5/16-18, 3/8-16, 7/16-14, 1/2-13, 9/16-12, 5/8-11, 3/4-10, 7/8-9, 1-8, custom]

//width of material to clamp
throat = 10;

// clamp depth of throat, 
depthMinimum=0; //1.0

// thickness of spine
spine_factor = 1; //0.1
//thickness 
thicknessFactor=1; //0.1

// moves trap towards spine
trap_backset = 1;
...
$thread_dia = 3; //0.01
$nut_flat = 5.5; //0.01
$nut_thickness = 2.15; //0.01
clearance = 0.3;

/* [Hidden] */

//small number for blending
eps = 0.001;// 

function inmm(x) = x*25.4;

// https://www.engineersedge.com/hardware/standard_metric_hex_nuts_13728.htm

//SAE Clearances from https://littlemachineshop.com/reference/tapdrill.php
//      nut    [cleardia, flat_dia, thick ]
nuts = [ 
        "M1.6",[1.6, 3.2,  1.3],
         "M2", [2,   4,    1.6],
       "M2.5", [2.5, 5,    2],
       ...
       "M3.5", [3.5, 6,    2.8 ],
         ...
         "M30",[30, 46,   25.6],
         "M36",[36, 55,   31],
         "#0-80",[inmm(0.0635), inmm(5/32),inmm(3/64)],
         "#1-64",[inmm(0.0760), inmm(5/32),inmm(3/64)],
         "#2-56",[inmm(0.0890), inmm(3/16),inmm(1/16)],
         "#3-48",[inmm(0.1040), inmm(3/16),inmm(1/16)],
         "#4-40",[inmm(0.1160), inmm(1/4),inmm(3/32)],
//         "#5-40",[inmm(0.1285), inmm(),inmm()],
         "#6-32", [inmm(0.144), inmm(5/16), inmm(7/64) ],
         "#8-32", [inmm(0.1659), inmm(11/32), inmm(1/8) ],
         "#10-24", [inmm(0.1960), inmm(3/8), inmm(1/8) ],
         "#12-24", [inmm(0.2210),inmm(7/16), inmm(5/32)],
         "1/4-20", [inmm(0.257), inmm(7/16), inmm(7/32) ],
         "5/16-18",[inmm(0.3230),inmm(1/2),inmm(17/64)],
         "3/8-16", [inmm(0.386),inmm(9/16),inmm(21/64)],
         "7/16-14",[inmm(0.4531),inmm(11/16),inmm(3/8)],
         "1/2-13", [inmm(0.5156),inmm(3/4),inmm(7/16)],
         "9/16-12",[inmm(0.5781),inmm(7/8),inmm(31/64)],
         "5/8-11", [inmm(0.6406),inmm(15/16),inmm(35/64)],
//         "11/16-24",[inmm(0.7031),inmm(xx),inmm()],
         "3/4-10", [inmm(0.7656),inmm(1+1/8),inmm(41/64)],
//         "13/16-20",[inmm(0.8281),inmm(),inmm()],
         "7/8-9",  [inmm(0.8906),inmm(1+5/16),inmm(3/4)],
//         "15/16-20",[inmm(0.9551),inmm(),inmm()],
         "1-8",[inmm(1+1/64),inmm(1+1/2),inmm(55/64)],
         "custom",[1,1,1]
       ];
...
nutdata = hash(nuts,nutname,"xx");
echo(str("OpenSCAD Version: ",version())); // Thingiverse is [2015,3,0]
// thingiverse reports:
//  ECHO: "Version: [2015, 3, 0]" 

thread_dia = (nutname != "custom")? nutdata[0]:$thread_dia;
nut_flat =  (nutname != "custom")? nutdata[1]:$nut_flat;
nut_thickness = (nutname != "custom")? nutdata[2]:$nut_thickness;

//calculate from flat diameter
nut_corner_diameter = nut_flat/cos(30);


c_clamp();
  
  //translate([0,-20,0])nut_pocket();
  
module c_clamp(){

    thickness = nut_corner_diameter *2 * thicknessFactor; 
    depth = max(depthMinimum,nut_corner_diameter+trap_backset+radius);
    spine_thickness = nut_thickness*3*spine_factor;
difference(){
    base(thickness,depth,throat,spine_thickness,radius,edge_break);



    translate([-1,1+nut_corner_diameter/2+trap_backset,thickness/2])
    rotate([0,-90,0])
       # nut_pocket(Bdia=thread_dia+clearance,
            Bminus=spine_thickness+10,Bplus=2,
            Pwide= nut_flat+clearance, Pdeep=nut_corner_diameter/2+clearance,
            Phigh= thickness, Pthick = nut_thickness+clearance);

//    #translate([-back_thickness-eps,nut_corner_diameter/2+deeper_trap,thickness/2
//    ])
//        rotate([0,90,0])
//            polyhole(h=back_thickness+2*eps,d=thread_dia+clearance);
//    #translate([-1-nut_thickness-clearance,(nut_corner_diameter-nut_flat)/2+deeper_trap,thickness/2-nut_corner_diameter/2])
//        cube([nut_thickness+clearance,nut_flat+clearance,thickness+nut_corner_diameter]);
    }
}


module nut_pocket(Bdia=3,Bplus=5,Bminus=10,Pwide=5.5,Pdeep=3.175,Phigh=10,Pthick=2.15){
    // defaults for an M3
    // Bore: dia, plus, minus
    // Pocket: wide,deep,high,thick
    translate([0,0,-Bplus])
        polyhole(h=Bplus+Bminus+2*eps,d=Bdia,center=false);
    translate([-Pdeep,-Pwide/2,0])
    cube([Pdeep+Phigh,Pwide,Pthick]); 
}


module base(thickness=10, depth=10,width=10, back_thickness=4,rounding=2,break=1){
    //
    
 //   assert (width >= rounding *2);
  radii_points=[
    [0,0,0],
    [0,depth,rounding],
    [width,depth,rounding],
    [width,0,0],
    [width+back_thickness,0,rounding+back_thickness],
    [width+back_thickness,depth+back_thickness,rounding+back_thickness],
    [-back_thickness,depth+back_thickness,rounding+back_thickness],
    [-back_thickness,0,0]
    ];
    
    polyRoundExtrude(radii_points,thickness,break,break);
}

//
//module cubic_cutter(h=1,l=10,d=1,steps=20,margin = 10,eps=0.001){
//    mypoints = [
//        for (a = [0 : steps])
//            [ a*l/steps, h*pow(((steps-a)/steps),(1/3)) ],
//        [l+eps,-margin],
//        [l+margin,-margin],
//        [l+margin,h+margin],
//        [-margin,h+margin],
...
//    
//    echo(mypoints);
//    translate([0,0,-eps])linear_extrude(d+2*eps)
//        polygon(mypoints);
//      //  square([l,h]);
//}
