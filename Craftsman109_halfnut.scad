...
module blockBlank(){
    translate([tiny/2,-blockDims[1]/2+tiny/2,tiny/20])
        cube(blockDims);
    translate([-blockDims[0]-tiny,-blockDims[1]/2+tiny/2,tiny/20+blockDims[2]+my_nut_thread_major()*1.25] )
        cube(blockDims);
    translate([-2*blockDims[0],-blockDims[1]+tiny,0])
        cube([4*blockDims[0],10,2*blockDims[2]+my_nut_thread_major()*1.25]);
...
difference(){
    blockBlank();
    translate([0,0,-tiny])
        my_thread(L=60+2*tiny);
}


module my_thread(L=20){
translate([0/2,0,0]){
straight_thread(my_nut_thread_profile(),
    pitch=my_nut_thread_pitch( ),r=my_nut_thread_major()/2,
    turns=L/my_nut_thread_pitch( ));
cylinder(d=my_nut_thread_major()- .9*my_nut_thread_height()+tiny,h=L);
    echo("hello ",my_nut_thread_major());
}
}



// my profile 1/2-16 square
function my_nut_thread_major()   = 1/2*25.4; // 
function my_nut_thread_pitch()   = 1/16*25.4;
function my_nut_thread_height()  = my_nut_thread_pitch();
function my_nut_thread_profile() = [
    [0,0],
    [-my_nut_thread_height(),0],
    [-my_nut_thread_height(),my_nut_thread_pitch()/2],
    [0,my_nut_thread_pitch()/2]
];


//polygon(my_nut_thread_profile());
